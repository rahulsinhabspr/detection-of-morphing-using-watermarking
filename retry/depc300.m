...
filename=('morphednewlivecoluredlimewatch.png');
giveninput=imread(filename);
% getting size
[s11,s22,s33]=size(giveninput);
%decreasing one row and column
s111=s11-1;
s222=s22-1;
s333=s33;
%creating new image with one row and column less
newgiveninput=zeros(s111,s222,s333);
newgiveninput=giveninput(1:s111,1:s222,1:s333);
%accessing last row
for j=1:1:s222
   row(1,j)=giveninput(s11,j); 
end
...
if (max(size(p))==5)
    ...
    if (max(size(q))==5)
        ...
        for j1=1:5
           for j2=1:5
              dp(j1,j2)= newgiveninput(q(j1),p(j2));
           end
        end
        ...
        for k=1:pq
            newgiveninput(q(1),p(k))=newgiveninput(q(1),p(k))+a1(k);
            newgiveninput(q(2),p(k))=newgiveninput(q(2),p(k))+a2(k);
            newgiveninput(q(3),p(k))=newgiveninput(q(3),p(k))+a3(k);
            newgiveninput(q(4),p(k))=newgiveninput(q(4),p(k))+a4(k);
            newgiveninput(q(5),p(k))=newgiveninput(q(5),p(k))+a5(k);
        end
        %Calculating new ne from modified image
        for j=1:s111
           for k=1:s222
              zdd(j,k)=mod(newgiveninput(j,k),10); 
           end
        end
