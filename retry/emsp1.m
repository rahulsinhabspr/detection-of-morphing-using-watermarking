...
input1=imread('magic_salad_plate','jpg');
input2=rgb2gray(input1);
inputb = rgb2gray(input1);
figure,imshow(inputb,[]);

[s1 s2]=size(input2);
n=s1*s2;


%finding the value which doesnot occur in last row and last column
rz=255;
for ir=1:s1
    ncc(ir,1)=inputb(ir,s2);
    if inputb(ir,s2)==255
       disp('rahul'); 
    end
end
...
imwrite(newimage,'encrypted_magic_salad_plate.png');
